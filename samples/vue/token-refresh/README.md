# Vue SPA — Token Refresh

Minimal Vue 3 app demonstrating token refresh using `oidc-client-ts` (via a `useAuth` composable) with the `offline_access` scope. Tokens are refreshed automatically before they expire, and can also be refreshed manually via a button.
